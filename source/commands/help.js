  ...
  async execute(interaction) {
    ...
    if (langDb == "en") {
      const helpembed = new MessageEmbed()
        .setAuthor(
          "Moderator Bot - Always be safe!",
          main.displaythings.cdn.bot_logo
        )
        .setTitle(`Hello ${interaction.user.username}`)
        .setDescription(
          `> **Hey! Need some help? Like commands, set values etc. Click buttons bellow for help!**`
        )
      ...
      const helpembed = new MessageEmbed()
        .setAuthor(
          "Moderator Bot - Her zaman güvende olun!",
          main.displaythings.cdn.bot_logo
        )
        .setTitle(`Merhaba ${interaction.user.username}`)
        .setDescription(
          `> **Hey! Biraz yardıma mı ihtiyacın var? Komutlar, değerleri ayarlamak v.b. gibi. Aşağıdaki butonlara tıklayarak yardım alabilirsin!**`
        )
        .setThumbnail(interaction.user.displayAvatarURL({ dynamic: true }))
        .setColor(main.displaythings.colors.color_main)
        .setFooter(
